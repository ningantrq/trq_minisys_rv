module minisys_soc (
  ...
  wire        pwm_reg_wr_w;
  wire        pwm_reg_rd_w;
  wire [31:0] pwm_reg_addr_w;
  wire [31:0] pwm_reg_data_wr_w;
  wire [31:0] pwm_reg_data_rd_w;
  
  // 看门狗信号定义
  wire        wdt_reg_wr_w;
  wire        wdt_reg_rd_w;
  wire [31:0] wdt_reg_addr_w;
  wire [31:0] wdt_reg_data_wr_w;
  wire [31:0] wdt_reg_data_rd_w;
  ...
  // PWM控制器实例化
  // ========================================================================
  // 功能：支持4路PWM输出，可独立配置周期、占空比和使能状态
  // 内存映射地址：0x70000000
  // 寄存器：周期/占空比寄存器、控制/状态寄存器
  peri_pwm pwm (
      .clk_i(clk_w),               // 系统时钟
      .rst_i(rst_i),               // 复位信号
  
      // 寄存器接口（来自peri_bridge）
      .reg_wr_i(pwm_reg_wr_w),     // 写使能
      .reg_rd_i(pwm_reg_rd_w),     // 读使能
      .reg_addr_i(pwm_reg_addr_w), // 寄存器地址
      .reg_data_wr_i(pwm_reg_data_wr_w), // 写数据
      .reg_data_rd_o(pwm_reg_data_rd_w), // 读数据
  
      // PWM输出端口
      .pwm0_o(pwm0_o),             // PWM通道0输出
      .pwm1_o(pwm1_o),             // PWM通道1输出
      .pwm2_o(pwm2_o),             // PWM通道2输出
      .pwm3_o(pwm3_o)              // PWM通道3输出
  );
  
  // ========================================================================
  // 看门狗定时器实例化
  // ========================================================================
  // 功能：可配置超时时间的看门狗定时器，支持中断和系统复位
  // 内存映射地址：0x90000000
  // 寄存器：超时时间寄存器、控制寄存器、状态寄存器、复位寄存器
  peri_watchdog watchdog (
      .clk_i(clk_w),               // 系统时钟
      .rst_i(rst_i),               // 复位信号
  
      // 寄存器接口（来自peri_bridge）
      .reg_wr_i(wdt_reg_wr_w),     // 写使能
      .reg_rd_i(wdt_reg_rd_w),     // 读使能
      .reg_addr_i(wdt_reg_addr_w), // 寄存器地址
      .reg_data_wr_i(wdt_reg_data_wr_w), // 写数据
      .reg_data_rd_o(wdt_reg_data_rd_w), // 读数据
  
      // 输出信号
      .interrupt_o(wdt_interrupt_o), // 中断输出
      .reset_o(wdt_reset_o)          // 复位输出
  );
endmodule