...
module tool_bram_cache (
  ...
  //J型指令测试（26-27）
  imem[26] = 32'h008000ef;  // JAL   x1, 8          x1=返回地址,跳过imem[27]
  imem[27] = 32'h00000013;  // NOP                   (被跳过)
  // 落地点：imem[28]

  //JALR测试（28-30）
  imem[28] = 32'h80000db7;  // LUI   x27,0x80000    x27=0x80000000
  imem[29] = 32'h07cd8d93;  // ADDI  x27,x27,124    x27=0x8000007C (imem[31]的地址)
  imem[30] = 32'h000d8e67;  // JALR  x28,x27,0      跳到x27地址(imem[31])
  // 不会执行到这里的指令

  //BEQ测试（31-33）：相等则跳转
  imem[31] = 32'h00500e93;  // ADDI  x29,x0, 5      x29=5
  imem[32] = 32'h005e8463;  // BEQ   x29,x5, 8      5==5?跳过imem[33]
  imem[33] = 32'h00000013;  // NOP                   (应被跳过)
  // 落地点：imem[34]

  //BNE测试（34-36）：不相等则跳转
  imem[34] = 32'h00a00e13;  // ADDI  x28,x0, 10     x28=10
  imem[35] = 32'h005e1463;  // BNE   x28,x5, 8      10!=5?跳过imem[36]
  imem[36] = 32'h00000013;  // NOP                   (应被跳过)
  // 落地点：imem[37]

  //BLT测试（37-40）：有符号小于则跳转
  imem[37] = 32'hfff00f13;  // ADDI  x30,x0, -1     x30=-1(有符号)
  imem[38] = 32'h00100f93;  // ADDI  x31,x0, 1      x31=1
  imem[39] = 32'h01ff4463;  // BLT   x30,x31, 8     -1<1?跳过imem[40]
  imem[40] = 32'h00000013;  // NOP                   (应被跳过)
  // 落地点：imem[41]

  //BGE测试（41-44）：有符号大于等于则跳转
  imem[41] = 32'h00500e93;  // ADDI  x29,x0, 5      x29=5
  imem[42] = 32'h00200e13;  // ADDI  x28,x0, 2      x28=2
  imem[43] = 32'h01ce5463;  // BGE   x28,x29, 8     2>=5?不跳，执行imem[44]
  imem[44] = 32'h00700e93;  // ADDI  x29,x0, 7      x29=7(BGE未跳转的证明)
  // 继续：imem[45]

  //BLTU测试（45-48）：无符号小于则跳转
  imem[45] = 32'h00300e13;  // ADDI  x28,x0, 3      x28=3
  imem[46] = 32'h00a00e93;  // ADDI  x29,x0, 10     x29=10
  imem[47] = 32'h01de6463;  // BLTU  x28,x29, 8     3<10u?跳过imem[48]
  imem[48] = 32'h00000013;  // NOP                   (应被跳过)
  // 落地点：imem[49]

  //BGEU测试（49-52）：无符号大于等于则跳转
  imem[49] = 32'hfff00f13;  // ADDI  x30,x0, -1     x30=0xFFFFFFFF(无符号最大)
  imem[50] = 32'h00100f93;  // ADDI  x31,x0, 1      x31=1
  imem[51] = 32'h01ff7463;  // BGEU  x30,x31, 8     0xFFFFFFFF>=1?跳过imem[52]
  imem[52] = 32'h00000013;  // NOP                   (应被跳过)
  // 落地点：imem[53]

  //分支测试完成标记
  imem[53] = 32'h00800e93;  // ADDI  x29,x0, 8      x29=8(标记分支测试完成)
  imem[54] = 32'h00000013;  // NOP
  imem[55] = 32'h00000013;  // NOP
  imem[56] = 32'h00000013;  // NOP
  imem[57] = 32'h00000013;  // NOP
